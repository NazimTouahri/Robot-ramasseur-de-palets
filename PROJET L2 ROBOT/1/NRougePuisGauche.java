...
/**
 *  Classe pour aller chercher les palets en partant de la ligne Rouge avec le premier virage a gauche
 * @author simon
 *
 */
public class NRougePuisGauche {
...
// TODO Auto-generated method stub
	...
	// les quelques lignes suivantes servent a empecher le robot de faire des mouvement hasardeux
ev3.avancer1();
Delay.msDelay(0);
ev3.arreter1(); 
ev3.avancer1();
...
/**
 * Permet d'aller chercher les palets en partant de la ligne Rouge avec le premier virage a gauche
 * Prend fin quand il n'y a plus de palets sur les intersections
 */
public static void ProjetRougePuisGauche() {
...
BonusGauche();
while(!color2.IsWhite()) {BonusGauche();}	// le robot faire l'arc de cercle jusqu'a la ligne blanche 
ev3.arreter1();
 ...
 * tournera ensuite vers la ligne verte , sur laquelle il tournera
...
pilot.rotate(-90);		// tourne vers la ligne bleu


/////////////RAMASSAGE DES PALETS SUR LA LIGNE BLEU/////////////////////


ev3.avancer1();
while(!color2.IsBlue()) {ev3.avancer1();}
ev3.arreter1();
...
while(!color2.IsBlue()) {Motor.A.forward();}		// Le robot tourne jusqu'au bleu
ev3.arreter1();
...
if(color2.IsBlue()) ev3.HorsLigne();
...
Motor.A.forward();
Motor.A.setSpeed(150);
while(!color2.IsBlue()) {Motor.A.forward();}
ev3.arreter1();

Motor.A.setSpeed(50);
Motor.A.forward();
Motor.A.setSpeed(50);
while(!color2.IsGrey()) {Motor.A.forward();}
ev3.arreter1();
Motor.A.setSpeed(300);
}
}
...
Delay.msDelay(800);	
ev3.arreter1();		// il s'arrete
Delay.msDelay(3000);	// il attend un petit peu
...
Motor.A.backward();
Motor.B.backward();		// recule un peu
Delay.msDelay(500);
ev3.arreter1();
...
/*
 * le robot va passer ensuite au ramssage des palets sur la ligne bleu
 * il devra d'abors tourner a 90 degré
...
ev3.avancer1();
Delay.msDelay(38);
ev3.arreter1();		// depasse un peu la ligne verte
...
Motor.A.forward();
Motor.A.setSpeed(150);
while(!color2.IsGreen()) {Motor.A.forward();}		// le robot tourne jusqu'au vert
ev3.arreter1();

Motor.A.setSpeed(50);		// on ralentis le moteur B a 50
Motor.A.forward();
Motor.A.setSpeed(50);
while(!color2.IsGrey()) {Motor.A.forward();}		// le robot tourne jusqu'au gris
ev3.arreter1();
...
while(!ultrasonic.TestUltrasonic2(0.15f))
{
	// les deux if permettent de suivre la ligne verte
if(color2.IsGreen()) ev3.HorsLigne();
if(color2.IsGrey()) ev3.SurLigne();
/*
 * si le robot rencontre un palet
 * il tourne a 90 degré , va deposer le palet derriere la ligne blanche
 * et reviendra sur la ligne bleu, sur laquelle il pivotera pour se mettre dans le bon axe 
 */
if(touch.isPressed()) 
{
ev3.arreter1();		//il s'arrete 	
...
ev3.avancer1();
Delay.msDelay(50);
ev3.arreter1();		// dépasse un petit peu la ligne verte
...
ev3.avancer1();
Delay.msDelay(50);
ev3.arreter1();		//depasse un peu le vert

//il effectue ensuite la rotation en deux temps pour bien se remettre dans l'axe de la ligne verte
